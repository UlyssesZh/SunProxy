...
jfieldID fidHostname = NULL;
jfieldID fidTls = NULL;

jobject create_packet_ulysses(const struct arguments *args,
                              jint version,
                              jint protocol,
                              const char *flags,
                              const char *source,
    ...
    jobject jpacket = create_packet(args, version, protocol, flags, source, sport, dest, dport, data, uid, 0);

    if (fidHostname == NULL) {
        fidHostname = jniGetFieldID(env, clsPacket, "hostname", "Ljava/lang/String;");
        fidTls = jniGetFieldID(env, clsPacket, "tls", "Z");
    }

    jstring jhostname = (*env)->NewStringUTF(env, hostname);
    ng_add_alloc(jhostname, "jhostname");

    (*env)->SetObjectField(env, jpacket, fidHostname, jhostname);
    (*env)->SetBooleanField(env, jpacket, fidTls, tls);
    (*env)->DeleteLocalRef(env, jhostname);

    ng_delete_alloc(jhostname, __FILE__, __LINE__);
