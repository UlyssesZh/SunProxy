...
/* Added by UlyssesZhan BEGIN */
        char *hostname = "";
        if (*server_name == 0 && protocol == IPPROTO_TCP) {
            const struct tcphdr *tcphdr = (struct tcphdr *) payload;
            const uint8_t tcpoptlen = (uint8_t) ((tcphdr->doff - 5) * 4);
            const uint8_t *payload_data = payload + sizeof(struct tcphdr) + tcpoptlen;
            const uint16_t datalen = (const uint16_t) (length - (payload_data - pkt));
            // This actually does not work because the HTTP header is incomplete in one packet,
            // but the socket is opened when handling the first packet when the hostname cannot be known.
            http_protocol->parse_packet(payload_data, datalen, &hostname);
        } else {
            hostname = server_name;
        }
        jobject objPacket = create_packet_ulysses(
                args, version, protocol, flags, source, sport, dest, dport, data, uid,
                hostname, use_tls);
/* Added by UlyssesZhan END */
